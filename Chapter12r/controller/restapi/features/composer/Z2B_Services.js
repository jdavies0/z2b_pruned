 ...
 * @returns {Order} - updated order course with all required fields except for relationships (student, seller)
 * @utility
 */
...
/**
 * find a course in an array. This is only used by the autoLoad process
 * @param {course_number} _courseUID - course number to find
 * @param {vendor_array} _courseArray - course array from order
 * @returns {JSON_object} - returns the course if found else error response
 * @utility
 */
    getCourse: function (_courseUID, _courseArray)
    {
        for (let each in _courseArray)
            { if (_courseArray[each].courseUID === _courseUID){return (_courseArray[each]);}}
        return({'description':'Course '+_courseUID+ 'Not Found', 'unitPrice': 0, 'extendedPrice': 0});
    },
/**
 * update course quantity. used by the autoLoad process.
 * @param {course_number} _courseUID - course number to find
 * @param {course_number} _qty - quantity to change * @utility
 * @param {vendor_array} _courseArray - course array from order
 */
    setCourse: function (_courseUID, _qty, _courseArray)
    {
        for (let each in _courseArray)
            {if (_courseArray[each].courseUID === _courseUID) {_courseArray[each].quantity += _qty;} }
    },
/**
 * supplemental routine to resubmit orders when MVCC_READ_CONFLICT encountered
 * @param {object} _con - web socket connection
 * @param {transaction} _course - transaction to process
 ...
 */
    loadTransaction: function (_con, _course, _id, businessNetworkConnection)
    {
        let method = 'loadTransaction';
        return businessNetworkConnection.submitTransaction(_course)
        .then(() => {
        ...
        .catch((error) => {
            if (error.message.search('MVCC_READ_CONFLICT') !== -1)
                ...
                this.loadTransaction(_con, _course, _id, businessNetworkConnection);
                }
            });
    },
/**
...
/**
 * saves the course table * @param {array} _table - array of JSON objects to save to file
 * @param {JSON} _table - data to be saved
 */
saveCourseTable: function (_table)
{
    console.log('_table: ', _table);
    let options = { flag : 'w' };
    let newFile = path.join(path.dirname(require.main.filename),'startup','courseList.txt');
    let _mem = '{"courses": [';
    for (let each in _table)
        {(function(_idx, _arr){if(_idx>0){_mem += ', ';} _mem += JSON.stringify(_arr[_idx]);})(each, _table);}
    _mem += ']}';
...
/**
 * update an empty order with 4 courses. update the amount field based on the sum of the line courses
 * @param {addCourses} _inbound - Order created with factory.newResource(NS, 'Order',.orderNumber)
 * @param {courseTable} _courseTable - arry of existing courses
 * @returns {Order} - updated order course with all required fields except for relationships (student, seller)
 * @utility
 */
addCourses: function (_inbound, _courseTable)
    {
        let _amount = 0;
        let _courses = [];
        let _this = this;
        for (let each in _inbound.courses)
            {(function(_idx, _arr)
                {
                    let _course = _this.getCourse(_arr[_idx].courseUID, _courseTable);
                    _this.setCourse(_arr[_idx].courseUID, _arr[_idx].quantity, _courseTable);
                    _arr[_idx].description = _course.courseDescription;
                    _arr[_idx].unitPrice = _course.unitPrice;
                    _arr[_idx].extendedPrice = _course.unitPrice*_arr[_idx].quantity;
                    _amount += _arr[_idx].extendedPrice;
                    _courses.push(JSON.stringify(_arr[_idx]));
                })(each, _inbound.courses);}
        return ({'courses': _courses, 'amount': _amount});
    },
/**
 ...
 * @param {Order} _order - the inbound Order course retrieved from a registry
...
{
    let orderElements = ['courses', 'status', 'amount', 'created', 'cancelled', 'bought', 'ordered', 'dateBackordered', 'requestShipment', 'delivered', 'delivering', 'approved',
