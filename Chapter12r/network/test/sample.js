...
const orderNo = '12345';
const studentID = 'billybob@email.com';
 ...
 * @utility
 */
function addCourses (_inbound)
{
	_inbound.items.push('{"courseUID":1,"courseID":"ACC-120","courseDescription":"Prin of Financial Accounting", "courseSection":"0001"  "creditHours":4, "term":"Spring 2019", "location":"Online","seats":34,"instructor","Cox,S"}');
	_inbound.items.push('{"courseUID":2,"courseID":"ACC-121","courseDescription":"Prin of Managerial Accounting", "courseSection":"0001"  "creditHours":4, "term":"Spring 2019", "location":"Online","seats":20, "instructor","Best,T}');
/*	_inbound.amount = JSON.parse(_inbound.items[0]).extendedPrice;
	_inbound.amount += JSON.parse(_inbound.items[1]).extendedPrice;
	_inbound.amount += JSON.parse(_inbound.items[2]).extendedPrice;
	_inbound.amount += JSON.parse(_inbound.items[3]).extendedPrice;
...
describe('Finance Network', function () {
    ...
    describe('#createOrder', () => {
        ...
        it('should be able to create an order', () => {
            ...
            student.companyName = 'billybob computing';
            ...
            order.student = factory.newRelationship(NS, 'Student', student.$identifier);
            ...
            // the student should of the commodity should be student
            //order.student.$identifier.should.equal(student.$identifier);
                ...
                .then((assetRegistry) => {
                    ...
                    return assetRegistry.add(order)
                        ...
                        .then((participantRegistry) => {
                            // add the student and seller
                            return participantRegistry.addAll([student, seller, shipper, provider]);
                        })
                        .then(() => {
                        ...
                        .then((newOrder) => {
                            ...
                            newOrder.student.$identifier.should.equal(studentID);
                        });
                });
        });
    });
        ...
        it('should be able to issue a buy request', () => {
            ...
            return businessNetworkConnection.getAssetRegistry(NS + '.Order')
                ...
                .then((newOrder) => {
                    ...
                    buyNow.student = newOrder.student;
        ...
        it('should be able to record a approval for order payment', () => {
            ...
            return businessNetworkConnection.getAssetRegistry(NS + '.Order')
                ...
                .then((newOrder) => {
                    ...
                    orderNow.order = factory.newRelationship(NS, 'Order', newOrder.$identifier);
                    orderNow.financeCo = factory.newRelationship(NS, 'FinanceCo', financeCoID);
                    orderNow.student = factory.newRelationship(NS, 'Student', newOrder.student.$identifier);
                    // submit the transaction
                    return businessNetworkConnection.submitTransaction(orderNow)
                        .then(() => {
                            return businessNetworkConnection.getAssetRegistry(NS + '.Order');
